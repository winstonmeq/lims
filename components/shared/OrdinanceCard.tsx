    ...
    <Card className="flex flex-col h-full hover:shadow-lg transition-shadow duration-300">
      <CardHeader>
        <div className="flex justify-between items-start gap-2">
          <CardTitle className="text-lg leading-tight">
            <Link href={`/ordinances/${ordinance._id}`} className="hover:text-primary transition-colors">
              {ordinance.title}
            </Link>
          </CardTitle>
          <StatusBadge status={ordinance.status} />
        </div>
        <CardDescription className="font-semibold text-primary">{ordinance.billNumber}</CardDescription>
      </CardHeader>
      <CardContent className="flex-grow">
        <p className="text-muted-foreground text-sm line-clamp-3">{ordinance.content}</p>
      ...
      <CardFooter>
        <Button asChild variant="link" className="p-0 h-auto text-primary">
          <Link href={`/ordinances/${ordinance._id}`}>
